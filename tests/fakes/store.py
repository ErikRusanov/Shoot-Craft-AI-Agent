...
Both fakes honor their ports' contracts (token-fenced locks, set-if-absent
idempotency, atomic-by-construction budget, live event tailing) but skip real
TTL expiry: ``ttl_seconds`` is accepted and ignored. They run the pipeline in a
single event loop, which is exactly why atomicity holds for free — no method
awaits between read and write, so concurrent coroutines never interleave a
check-and-increment.
"""
...
class InMemoryEventBus:
    ...
    the fake exercises the same (de)serialization a real bus would, and stream
